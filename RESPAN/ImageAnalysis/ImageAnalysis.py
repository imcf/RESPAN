...
from skimage import (  # graph #util,  color, data, filters,  exposure, restoration
    exposure,
    measure,
    morphology,
    segmentation,
    ...
    for file in range(len(files)):
        ...
        if settings.dask_enabled == True:
            if settings.resave_omezarr:
                logger.info(
                    f"  Resaving files as OME.Zarr for quick access using Dask..."
                )
            ...
            if len(labels.shape) == 4:  # Check if it has a channel dimension (CZYX)
                ...
                logger.info(
                    f"  Removed channel dimension from labels, new shape: {labels.shape}"
        ...
        if image_GB > 2 and settings.resave_omezarr == False:
            logger.info(
                f"  *Note, as the dataset is over 2GB, we recommend enabling Dask for efficient processing. "
            )
        ...
        if settings.axial_restore == True:
            ...
            logger.info(
                f"  Due to axial restoration, image rescaled to match labels: {image.shape}"
            )
        ...
        if settings.axial_restore == False:
            if (
                settings.original_shape[0] != None
                and settings.input_resZ != settings.model_resZ
                or settings.input_resXY != settings.model_resXY
                and settings.prev_labels == False
            ):
                logger.info(
                    f"  orignal settings shape: {settings.original_shape[file]}"
                )
                labels = resize(
                    labels,
                    settings.original_shape[file],
                    order=0,
                    mode="constant",
                    preserve_range=True,
                    anti_aliasing=None,
                ...
                imwrite(
                    locations.labels + label_files[file],
        ...
        # else:
        if settings.dask_enabled == True:
            spine_summary = dia.spine_and_whole_neuron_processing(
                image,
                labels,
                spine_summary,
                raw_zarr_root,
            ...
            spine_summary = spine_and_whole_neuron_processing(
                image,
                labels,
                spine_summary,
                settings,
        ...
        if settings.shape_error == True:
            logger.info(
                f"!!! One or more images moved to \\Not_Processed due to having\nless than 5 Z slices. Please modify these files before reprocessing.\n"
            )

        spine_summary.to_csv(
            locations.tables + "Detected_spines_summary.csv", index=False
        )
...
def spine_and_whole_neuron_processing(
    image, labels_vol, spine_summary, settings, locations, filename, log, logger
):
    ...
    if np.max(labeled_dendrites) > 0:
        ...
        dendrites_mask = (labeled_dendrites > 0).astype(
            np.uint8
        ...
        spine_table, spines_filtered = initial_spine_measurements(
            image,
            spine_labels,
            1,
            0,
        ...
        if settings.neck_generation == False:
            ...
            if settings.model_type >= 3:
                ...
                target = (
                    dendrites_mask + (remaining_necks > 0)
                ...
                extended_necks = extend_objects_GPU(
                    neck_labels_updated, target, neuron, settings, locations, logger
                )
                ...
                connected_necks = associate_spines_with_necks_gpu(
                    (extended_necks + neck_labels_updated), remaining_necks, logger
                )
                ...
                connected_necks = extend_objects_GPU(
        ...
        if settings.second_pass:
            ...
            if settings.refinement_model_path != None:
                spines_filtered, connected_necks = second_pass_annotation(
                    spines_filtered,
                    connected_necks,
                    dendrites_mask,
                    neuron,
        ...
        dendrite_lengths, dendrite_volumes, skeleton_coords, labeled_skeletons = (
            calculate_dendrite_length_and_volume_fast(
        ...
        if np.max(soma) == 0:
            spine_dendID_and_geodist, geodesic_distance_image = (
                calculate_dend_ID_and_geo_distance(
                    labeled_dendrites,
                    spines_filtered,
                    skeleton_coords,
                    labeled_skeletons,
        ...
        if settings.additional_logging:
            logger.info(
                f"      Max ID for spines filtered is {np.max(spines_filtered)}"
            )
            logger.info(
                f"      Max ID for connected necks is {np.max(connected_necks)}"
            )

        if settings.mesh_analysis == True:
            spine_mesh_results = analyze_spines_batch(
        ...
        if len(spine_table) == 0 and np.max(labeled_dendrites) == 0:
            ...
            spine_head_table, spines_filtered = spine_vox_measurements(
                image,
                spines_filtered,
                1,
                0,
            ...
            spine_whole_table, spines_filtered = spine_vox_measurements(
                image,
            ...
            neck_table, spines_filtered = spine_vox_measurements(
                image,
                connected_necks,
                1,
                0,
            ...
            spine_dendID_and_geodist["geodesic_dist_to_soma"] = (
                spine_dendID_and_geodist["geodesic_dist_to_soma"] * settings.input_resXY
            )
            ...
            if spine_head_table.empty:
                logger.info(
                    "   Note some spine head meshes couldn't be generated, reverting those to voxel based"
                )
                ...
                for col in spine_table.columns:
                    if col.startswith(f"{prefix}_C") and col.endswith("_mean_int"):
                        ...
                        if id_col not in spine_table.columns:  # avoid overwriting
                            spine_table[id_col] = (
                                spine_table[vol_col] * spine_table[col]
                            )
    ...
    logger.info(
        f" Processing complete for file {filename}. Total spine analysis time: {round(total_time, 2)} minutes.\n---"
    )
...
@mp.profile_mem()
def spine_detection_cores_and_membranes(
    cores, membranes, remove_borders, logger, settings
):
    """
    ...
    labeled_spines = segmentation.watershed(
        -distance, markers=labeled_cores, mask=cores | membranes
    )

    logger.info(
        f"    Voxels unassigned to spines: {np.sum(membranes & ~(labeled_spines > 0))}."
    )
...
def calculate_dendrite_length_and_volume_fast(labeled_dendrites, skeleton, logger):
    ...
    unique_labels_in_skeleton = unique_labels_in_skeleton[
        unique_labels_in_skeleton != 0
    ]
    ...
    lengths = ndimage.sum(
        np.ones_like(labeled_skeletons),
        labeled_skeletons,
        index=unique_labels_in_skeleton,
    )
    ...
    dendrite_volumes = ndimage.sum(
        np.ones_like(labeled_dendrites), labeled_dendrites, index=unique_dendrite_labels
    )
...
@mp.profile_mem()
def initial_spine_measurements(
    image,
    labels,
    dendrite,
    max_label,
...
def associate_spines_with_necks_gpu(spines, necks, logger):
    ...
    while growth_occurred:
        ...
        for label_id in cp.unique(labeled_array_cp):
            ...
            expanded_labels_cp = cp.where(
                dilated_label_mask & new_growth_cp, label_id, expanded_labels_cp
            )
...
def extend_single_object_GPU_v2(
    label_value,
    object_subvolume,
    target_subvolume,
    intensity_subvolume,
    ...
    intensity_subvolume_gpu = pad_subvolume_gpu(
    ...
    path, distance_map = pathfinding_v3b(
        object_subvolume_gpu, target_subvolume_gpu, intensity_subvolume_gpu, logger
    )
...
def analyze_spines_batch(
    ...
    mem_required, free_mem, total_mem = check_gpu_memory_requirements(
        spine_labels_vol, logger
    )
    # estimate requirements

    if mem_required > 0.25 * total_mem:
        logger.info(
            "       Volume exceeds 25% of GPU memory. Implementing tiling strategy."
        )

        return analyze_spines_batch_tiled(
            spine_labels_vol,
            head_labels_vol,
            dendrite,
            neuron,
    ...
    cp_multi_channel = cp.stack(
        [
            cp.asarray(spine_labels_vol),
    ...
    batch_size = calculate_batch_size(
        mem_required, spine_labels_vol, avg_sub_volume_shape, dtype_size
    )
    ...
    # batch
    total_batches = (
        len(labels) + batch_size - 1
    ) // batch_size  # Calculate total number of batches (ceiling division)
    milestones = [
        int(total_batches * p / 100) for p in range(10, 101, 10)
    ]  # Create milestones at 10% intervals
    ...
    for i in range(0, len(labels), batch_size):
        ...
        if current_batch in milestones:
            ...
            logger.info(
                f"        Processing batch {current_batch} of {total_batches}... ({int(percentage)}% complete)"
            )

        # Extract subvolumes
        sub_volumes, start_coords = extract_subvolumes_mulitchannel_GPU_batch(
            cp_multi_channel, batch_labels
        )
        ...
        padded_subvolumes, pad_amounts = pad_and_center_subvolumes(
            sub_volumes, settings, logger
        )
        ...
        export_subvols = cp.transpose(
            cp.stack(padded_subvolumes, axis=0), (0, 1, 4, 2, 3)
        )
        ...
        imwrite(
            f"{locations.arrays}/Spine_vols_{settings.filename}_b{i // batch_size + 1}.tif",
            export_subvols.astype(np.uint16),
            compression="zlib",
            compressionargs=1,
            imagej=True,
            photometric="minisblack",
        ...
        imwrite(
            f"{locations.arrays}/Spine_MIPs_{settings.filename}_b{i // batch_size + 1}.tif",
            np.max(export_subvols, axis=1).astype(np.uint16),
            compression="zlib",
            compressionargs=1,
            imagej=True,
            photometric="minisblack",
        ...
        if settings.additional_logging == True:
            logger.info(
                f"Subvolume 0 shape after padding: {padded_subvolumes[0].shape}"
            )
        ...
        batch_data = cp.stack(
            [subvol[:, :, :, 0].astype(cp.float32) for subvol in padded_subvolumes]
        )  # spine volumes
        batch_dendrite = cp.stack(
            [subvol[:, :, :, 2] for subvol in padded_subvolumes]
        )  # dendrite
        # binary of batch_dendrite
        batch_binary = cp.stack(
            [subvol[:, :, :, 2] > 0 for subvol in padded_subvolumes]
        )  # dendrite binary
        ...
        spine_results, midlines = batch_mesh_analysis(
            batch_data,
            start_coords,
            closest_points,
            pad_amounts,
        ...
        if settings.additional_logging == True:
            logger.info(
                f"      Time taken for mesh measurements of spines: {time.time() - t0:.2f} seconds"
            )

        # Subtract heads from spines and measure necks and ensure float for marching_cubes
        batch_data = cp.stack(
            [
                cp.maximum(subvol[:, :, :, 0] - subvol[:, :, :, 1], 0).astype(
                    cp.float32
                )
        ...
        neck_results, _ = batch_mesh_analysis(
            batch_data,
            start_coords,
            closest_points,
            pad_amounts,
            ...
            settings,
            logger,
        )  # True for min max mean width
        if settings.additional_logging == True:
            logger.info(
                f"      Time taken for mesh measurements of necks: {time.time() - t0:.2f} seconds"
            )

        # now process heads
        t0 = time.time()
        batch_data = cp.stack(
            [subvol[:, :, :, 1] for subvol in padded_subvolumes]
        )  # spine vols
        # Process just heads - but don't need vol for length just surface area
        head_results, _ = batch_mesh_analysis(
            batch_data,
            start_coords,
            closest_points,
            pad_amounts,
            ...
            settings,
            logger,
        )  # False for width using bounding box
        if settings.additional_logging == True:
            logger.info(
                f"      Time taken for mesh measurements of spine heads: {time.time() - t0:.2f} seconds"
            )
    ...
    logger.info(
        f"      Total time taken for mesh analysis: {time.time() - start_time:.2f} seconds"
    )
...
@mp.profile_mem()
def calculate_dend_ID_and_geo_distance(
    labeled_dendrites,
    labeled_spines,
    skeleton_coords,
    skeleton_labels,
    ...
    try:
        ...
        geodesic_distance_image = np.full(
            labeled_dendrites.shape, np.nan, dtype=np.float32
        )
            ...
            if soma_vol is not None:
                ...
                if c_coords.size > 0:
                    ...
                    starting_point = tuple(
                        object_coords[np.argmin(np.sum(object_coords, axis=1))]
                    )
            else:
        ...
        for label_B, distance in geodesic_distances_B.items():
            dend_IDs.loc[dend_IDs["label"] == label_B, "geodesic_dist_to_soma"] = (
                distance
            )
        ...
        geodesic_distance_image = np.nan_to_num(geodesic_distance_image, nan=0).astype(
            np.float32
        )
        ...
        #    imwrite(locations.MIPs+filename+'_geodesic_dist.tif', geodesic_distance_image, compression='zlib', compressionargs=1)
...
@mp.profile_mem()
def analyze_spines_batch_tiled(
    spine_labels_vol,
    head_labels_vol,
    dendrite,
    neuron,
    ...
    total_tiles = (
        math.ceil(shape[0] / (tile_size[0] - overlap[0]))
    ...
    for z in range(0, shape[0], tile_size[0] - overlap[0]):
        for y in range(0, shape[1], tile_size[1] - overlap[1]):
            for x in range(0, shape[2], tile_size[2] - overlap[2]):
                ...
                logger.info(
                    f"      Processing tile {current_tile} of {total_tiles}: z={z}:{z_end}, y={y}:{y_end}, x={x}:{x_end}"
                )

                # Process the tile
                tile_results = process_spine_batch_tile(
                    f"tile{z}_{y}_{x}",
                    tile_spine,
                    tile_head,
                    tile_dendrite,
                ...
                for result in tile_results:
                    result["start_coords"] = [
                        coord + offset
                        for coord, offset in zip(result["start_coords"], [z, y, x])
                    ]
    ...
    logger.info(
        f"      Total time taken for tiled spine mesh analysis: {time.time() - start_time:.2f} seconds"
    )
    return spine_results_df
...
def spine_vox_measurements(
    ...
    for ch in range(image.shape[1] - 1):
        ...
        # rename mean intensity
        table.rename(
            columns={"mean_intensity": f"{prefix}_C" + str(ch + 2) + "_mean_int"},
            inplace=True,
        )
        table.rename(
            columns={"max_intensity": f"{prefix}_C" + str(ch + 2) + "_max_int"},
            inplace=True,
        )
    ...
    if prefix == "head":
        if settings.use_vox_measurements == False:
            ...
            try:
                ...
                logger.info(
                    f"                Convex hull calculation failed: {str(e)}. Filtering problematic spines and retrying.\n    "
                ...
                try:
                    labels, num_valid = filter_invalid_objects(
                        labels, min_volume=settings.neuron_spine_size[0], min_dims=3
                    )
                    distance_table = pd.DataFrame(
                        measure.regionprops_table(
                            labels,
                    ...
                    logger.info(
                        f"                Convex hull calculation failed again: {str(e2)}. Using fallback properties."
                    )
                    distance_table = pd.DataFrame(
                        measure.regionprops_table(
                            labels,
        ...
        distance_table.rename(
            columns={"min_intensity": "head_euclidean_dist_to_dend"}, inplace=True
        )
        distance_table.rename(
            columns={"max_intensity": "spine_length_euclidean"}, inplace=True
        )
        ...
        if settings.use_vox_measurements == True:
            ...
            distance_table.rename(
                columns={"solidity": "head_solidity_vox"}, inplace=True
            )
            distance_table.rename(
                columns={"area_convex": "head_vol_convex_vox"}, inplace=True
            )
            distance_table.rename(
                columns={"axis_major_length": "head_major_length_vox"}, inplace=True
            )
            distance_table.rename(
                columns={"axis_minor_length": "head_minor_length_vox"}, inplace=True
            )

            distance_col = (
                distance_table["head_bbox_vox"]
                * settings.input_resXY
                * settings.input_resXY
                * settings.input_resZ
            )
            ...
            distance_col = (
                distance_table["head_vol_convex_vox"]
                * settings.input_resXY
                * settings.input_resXY
                * settings.input_resZ
            )
            ...
            distance_col = (
                distance_table["head_major_length_vox"] * settings.input_resXY
            )
            main_table = main_table.join(distance_col)

            distance_col = (
                distance_table["head_minor_length_vox"] * settings.input_resXY
            )
            main_table = main_table.join(distance_col)
        ...
        distance_col = (
            distance_table["head_euclidean_dist_to_dend"] * settings.input_resXY
        )
        main_table = main_table.join(distance_col)
        ...
        if np.max(soma_distance) > 0:
            ...
            soma_distance_table.rename(
                columns={"min_intensity": "euclidean_dist_to_soma"}, inplace=True
            )
            distance_col = (
                soma_distance_table["euclidean_dist_to_soma"] * settings.input_resXY
            )
            main_table = main_table.join(distance_col)
        else:
    ...
    if prefix == "spine" and settings.use_vox_measurements == True:
        try:
            ...
            logger.info(
                f"                Convex hull calculation failed for some spines: {str(e)} Filtering problematic spines and retrying."
            )
            try:
                labels, num_valid = filter_invalid_objects(
                    labels, min_volume=settings.neuron_spine_size[0], min_dims=3
                ...
                logger.info(
                    f"                Convex hull calculation failed for some spines: {str(e2)} Using fallback properties."
                )
                distance_table = pd.DataFrame()
        ...
        distance_table.rename(
            columns={"axis_major_length": "spine_major_length_vox"}, inplace=True
        )
        distance_table.rename(
            columns={"axis_minor_length": "spine_minor_length_vox"}, inplace=True
        )

        distance_col = (
            distance_table["spine_bbox_vox"]
            * settings.input_resXY
            * settings.input_resXY
            * settings.input_resZ
        )
    ...
    for prop in props:
        ...
        if len(bbox) == 6:  # 3D case
            ...
            smallest_to_largest_ratio = (
                sorted_dims[0] / sorted_dims[2] if sorted_dims[2] > 0 else 0
            )
...
def check_image_shape(image, logger):
    if len(image.shape) > 3:
        ...
        if smallest_axis != 1:
            ...
            logger.info(
                f"   Channels moved ZCYX - raw data now has shape {image.shape}"
            )  # ImageJ supports TZCYX order
    else:
        image = np.expand_dims(image, axis=1)
...
def pathfinding_v3b(
    object_subvolume_gpu, target_subvolume_gpu, intensity_image_gpu, logger
):
    """
...
def calculate_neck_length_and_width(skeleton_points, verts):
    ...
    @cp.fuse()
    def perpendicular_distance_xy(
        point_y, point_x, skeleton_y, skeleton_x, tangent_y, tangent_x
    ):
    ...
    for i, (skeleton_point, tangent) in enumerate(
        zip(smooth_skeleton_points, tangents)
    ):
...
def mesh_neck_width_and_length(volume, verts, logger, scaling):
    ...
    if len(skeleton_points) < 2:
        ...
        projections = (
            cp.dot(diffs, skeleton_axis)[:, None] * skeleton_axis
        )  # Project onto axis
        ...
        total_length, min_width, max_width, mean_width, skeleton_output = (
            calculate_neck_length_and_width(skeleton_points, verts)
        )
...
def mesh_neck_width_and_length_midline_calculated(midline, verts, logger, scaling):
    ...
    total_length = cp.sum(
        cp.sqrt(cp.sum(cp.diff(midline * scaling, axis=0) ** 2, axis=1))
    )
...
def mesh_neck_width_and_length_closest(volume, verts, closest_point, logger, scaling):
    ...
    boost_map = cp.asarray(
        gaussian_filter(cp.asnumpy(boost_map), sigma=gaussian_boost_radius)
    )

    # Now modify the cost map with a stronger Gaussian boost
    cost_map = (
        cost_map * (1.0 - boost_map * 2) + boost_map * 0.1
    )  # More aggressive scaling near furthest point
    ...
    try:
        indices, _ = route_through_array(
            cost_map_np, closest_point, furthest_point_np, fully_connected=True
        )
        if not indices:
    ...
    scaled_diff = (
        diff * scaling
    )  # Element-wise multiplication of voxel differences by the corresponding scaling factors
...
def extract_subvolumes_mulitchannel_GPU_batch(multi_channel_array, labels, padding=5):
    ...
    for idx, label in enumerate(labels):
        ...
        start = cp.maximum(cp.min(coords, axis=0) - padding, 0)
            ...
            cp.max(coords, axis=0) + padding + 1,
            cp.array(multi_channel_array.shape[:3]),
        )

        subvolume = multi_channel_array[
...
def extract_subvolumes_mulitchannel_GPU_batch_2ndpass(
    multi_channel_array, labels, padding=5
):
    subvolumes = []
    start_coords = []
...
def check_gpu_memory_requirements(spine_labels_vol, logger):
    ...
    logger.info(
        f"       Total memory required for full multi-channel array: {total_memory_required / 1e9:.2f} GB"
    )
    logger.info(
        f"       Total available GPU memory: {free_mem / 1e9:.2f} GB. Percentage of GPU memory required for array: {(total_memory_required / free_mem) * 100:.2f}%"
    )
...
def process_spine_batch_tile(
    ...
    free_mem, _ = cp.cuda.runtime.memGetInfo()
    batch_size = calculate_batch_size(
        mem_required, tile_spine, avg_sub_volume_shape, dtype_size
    )
    ...
    for i in range(0, len(labels), batch_size):
        ...
        imwrite(
            f"{locations.arrays}/Spine_vols_{settings.filename}_tile{tile}_b{i // batch_size + 1}.tif",
            export_subvols.astype(np.uint16),
            compression="zlib",
            compressionargs=1,
            imagej=True,
        ...
        imwrite(
            f"{locations.arrays}/Spine_MIPs_{settings.filename}_tile{tile}_b{i // batch_size + 1}.tif",
            np.max(export_subvols, axis=1).astype(np.uint16),
            compression="zlib",
            compressionargs=1,
            imagej=True,
...
def find_closest_dendrite_id(closest_points, batch_dendrite):
    ...
    for closest_point, dendrite in zip(closest_points, batch_dendrite):
        ...
        closest_dendrite_point = cp.unravel_index(
            cp.argmin(masked_dist), dendrite.shape
        )
...
def pad_subvolumes(subvolumes, logger):
    ...
    for subvol in subvolumes:
        pad_width = [
            (0, max_dim - dim) for max_dim, dim in zip(max_shape, subvol.shape)
        ]
...
def pad_and_center_subvolumes(subvolumes, settings, logger):
    ...
    for subvol in subvolumes:
        ...
        pad_width = [
            (max_dim - dim) // 2 + ((max_dim - dim) % 2 > 0)
            for max_dim, dim in zip(max_shape, subvol.shape)
        ]

        # Create the full pad_width tuple for np.pad
        full_pad_width = [
            (pad, max_dim - dim - pad)
            for pad, max_dim, dim in zip(pad_width, max_shape, subvol.shape)
        ]

        padded_subvol = np.pad(
            subvol, full_pad_width, mode="constant", constant_values=0
        )
        padded_subvolumes.append(padded_subvol)
...
def unpad_result(result, pad_amount):
    ...
    result["start_coords"] = [
        coord - pad for coord, (pad, _) in zip(result["start_coords"], pad_amount)
    ]
    return result


def batch_mesh_analysis(
    batch_data,
    start_coords,
    closest_points,
    pad_amounts,
    ...
    for i, (
        ...
        if cp.issubdtype(subvol.dtype, cp.floating):
            if cp.isnan(subvol).any() or cp.isinf(subvol).any():
                logger.error(
                    f"subvol contains NaN or Inf values for object ID {obj_id}"
                )
                # Handle the error as appropriate
        ...
        try:
            ...
            logger.error(
                f"Error calculating voxel count for {prefix} {i} spine {obj_id}: {str(e)}"
            )
            voxel_count = 0
        ...
        try:
            ...
            if len(verts) == 0 or len(faces) == 0:
                ...
                result = create_empty_result(
                    obj_id, start_coord, closest_point, prefix, multiple_widths_option
                )
            ...
            if multiple_widths_option:
                if voxel_count == 1:
                    length, min_width, max_width, mean_width = (
                        1 * scaling[1],
                        1 * scaling[1],
                        1 * scaling[1],
                        1 * scaling[1],
                ...
                if calc_midline and voxel_count > 1:
                    length, min_width, max_width, mean_width, skeleton_result = (
                        mesh_neck_width_and_length_closest(
                    ...
                    if midline is not None and len(midline) > 0 and voxel_count > 1:
                        try:
                            (
                                ...
                                mean_width,
                                skeleton_result,
                            ) = mesh_neck_width_and_length_midline_calculated(
                                midline, verts, logger, scaling
                            )
                        except Exception as e:
                            logger.error(
                                f"Unexpected error calculating with predifined midline {prefix} {i} spine {obj_id}: {str(e)}"
                            )
                            ...
                            logger.info(
                                f"Skeleton result is {skeleton_result} for {prefix} {i}"
                            )
                ...
                result = {
                    "ID": obj_id,
                    "start_coords": start_coord.get().tolist()
                    if closest_point is None
                    else (start_coord + closest_point).get().tolist(),
            ...
            if settings.save_intermediate_data:
                mesh_filename = (
                    f"{locations.Meshes}/{settings.filename}/{prefix}/{obj_id}.obj"
                )
                ...
                if skeleton_result is not None:
                    ...
                    for point in skeleton_voxels:
                        point = tuple(point)
                        if np.all(np.array(point) >= 0) and np.all(
                            np.array(point) < np.array(skeleton_channel.shape)
                        ):
                    ...
                    multi_channel_image = np.stack(
                        [subvol_data, skeleton_channel, closet_point_channel], axis=1
                    )
                    multi_channel_image = multi_channel_image * 65535

                    imsave_filename = (
                        f"{locations.Meshes}/{settings.filename}/{prefix}/{obj_id}.tif"
                    )
                    imwrite(
                        imsave_filename,
                        multi_channel_image.astype(np.uint16),
                        compression="zlib",
                        compressionargs=1,
                        imagej=True,
            ...
            logger.error(
                f"Unexpected error processing {prefix} {i} spine {obj_id}: {str(e)}"
            )
...
def create_empty_result(
    ...
    if multiple_widths_option:
        ...
        base_result.update({f"{prefix}_length": 0.0, f"{prefix}_width": 0.0})
...
def filter_dendrites(dendrites, settings, logger):
    ...
    dend_vols = ndimage.sum_labels(
        dendrites, dendrite_labels, index=range(1, num_detected + 1)
    )
    large_dendrites = dend_vols >= settings.min_dendrite_vol

    # Create new dendrite binary and relabel
    filt_dendrites, num_filtered = ndimage.label(
        np.isin(dendrite_labels, np.nonzero(large_dendrites)[0] + 1)
    )
...
def filter_dendrites_cupy(
    dendrites_np, settings, logger, connectivity=3
):  # 3 => 26‑conn in 3‑D
    """
    Parameters
    ----------
    ...
    filt_labels_cp, num_filtered = cp_ndimage.label(filt_binary_cp, structure=struct)
...
def adaptive_distance_transform(image, logger, threshold_size=20 * 2000 * 2000):
    ...
    logger.info(
        f"    Time taken for adaptive distance calculation: {time.time() - time_initial:.2f} seconds"
    )
    return large_distance


def categorize_spine(
    L,
    dH,
    dN,
...
def create_kdtree_from_skeleton(
    skeleton_coords, skeleton_labels, sampling_method="systematic", sampling_param=4
):

    print(
        f"Skeleton points: {skeleton_coords.shape[0]}, Skeleton labels: {skeleton_labels.shape[0]}"
    )
...
def downsample_skeleton_points_random(
    skeleton_points, skeleton_labels, sampling_rate=0.25
):
...
def voxel_grid_filter(skeleton_points, skeleton_labels, voxel_size=2):
    ...
    keys = quantized_coords.view(
        [("", quantized_coords.dtype)] * quantized_coords.shape[1]
    )
...
def match_and_relabel_objects_geo(tree_A, labels_A, image_B, max_distance=None):
    ...
    label_map_dict = {}
    for spine_label, dendrite_label, is_valid in zip(
        labels_B_1D, closest_labels, valid
    ):
        if is_valid:
    ...
    dend_IDs = pd.DataFrame({"label": labels_B_1D, "dendrite_id": closest_labels})
...
def analyze_spines_4D(settings, locations, log, logger):
    ...
    #
    spine_labels = spine_detection_4d(
        spines, settings.erode_shape, settings.remove_touching_boarders, logger
    )  # value used to remove small holes
    ...
    for t in range(labels.shape[0]):
        ...
        # Calculate volumes and filter
        dend_vols = ndimage.sum_labels(
            dendrites_3d, dendrite_labels, index=range(1, num_detected + 1)
        )

        large_dendrites = dend_vols >= settings.min_dendrite_vol

        # Create new dendrite binary
        dendrites_3d = np.isin(
            dendrite_labels, np.nonzero(large_dendrites)[0] + 1
        ).astype(bool)

        filt_dendrites = np.max(measure.label(dendrites_3d))

        logger.info(
            f"  Processing{filt_dendrites} of {num_detected} detected dendrites larger than minimum volume threshold of {settings.min_dendrite_vol * settings.input_resXY * settings.input_resXY * settings.input_resZ} µm<sup>3</sup> or {settings.min_dendrite_vol} voxels..."
        )
        ...
        if filt_dendrites > 0:
            ...
            dendrite_distance = ndimage.distance_transform_edt(
                np.invert(dendrites_3d)
            )  # invert neuron mask to get outside distance
    ...
    for t in range(labels.shape[0]):
        ...
        # max label for 4d?
        spine_table, spines_filtered = spine_measurementsV2(
            image[t, :, :, :],
            spine_labels[t, :, :, :],
            0,
            0,
        ...
        if len(spine_table) == 0 and np.max(dendrites[t, :, :, :]) == 0:
            ...
            spine_MIPs, spine_slices, spine_vols = create_spine_arrays_in_blocks(
                image[t, :, :, :],
                labels[t, :, :, :],
                spines_filtered,
                spine_table,
                settings.roi_volume_size,
                settings,
                locations,
                str(t + 1) + ".tif",
                logger,
                settings.GPU_block_size,
            )
            ...
            spine_ids = np.nan_to_num(
                np.sum(label_areas * spine_masks, axis=(1, 2))
                / np.sum(spine_masks, axis=(1, 2), where=spine_masks)
            )
            ...
            spine_table.insert(
                5, "spine_area", spine_table.pop("spine_area")
            )  # pops and inserts
            # spine_table.insert(5, 'spine_area', df_spine_areas['spine_area'])
            spine_table.insert(
                6,
                "spine_area_um2",
                spine_table["spine_area"] * (settings.input_resXY**2),
            )
            ...
            # append tables
            all_spines_table = pd.concat(
                [all_spines_table, spine_table], ignore_index=True
            )

            # all_spines_table.to_csv(locations.tables + str(t)+ 'Detected_spines_appened.csv',index=False)

            summary = tables.create_spine_summary_neuron(
                spine_table, str(t + 1), dendrite_length, dendrite_volume, settings
            )
            ...
            # Append to the overall summary DataFrame
            all_summary_table = pd.concat(
                [all_summary_table, summary], ignore_index=True
            )
            # append images
    ...
    vol_over_t = all_spines_table.pivot(
        index="spine_id", columns="timepoint", values="spine_vol"
    )
    ...
    all_summary_table.to_csv(
        locations.tables + "Detected_spines_4D_summary.csv", index=False
    )
...
def create_filtered_and_unfiltered_spine_arrays_cupy(
    image,
    spines_filtered,
    labels,
    table,
    ...
    image_cp = cp.pad(
        image_cp,
        (
            (volume_size_z // 2, volume_size_z // 2),
            (volume_size_y // 2, volume_size_y // 2),
            (volume_size_x // 2, volume_size_x // 2),
            (0, 0),
        ),
        mode="constant",
        constant_values=0,
    )
    spines_filtered_cp = cp.pad(
        spines_filtered_cp,
        (
            (volume_size_z // 2, volume_size_z // 2),
            (volume_size_y // 2, volume_size_y // 2),
    ...
    for index, row in table.iterrows():
        ...
        image_vol = image_cp[
            z_min + volume_size_z // 2 : z_max + volume_size_z // 2,
            y_min + volume_size_y // 2 : y_max + volume_size_y // 2,
            x_min + volume_size_x // 2 : x_max + volume_size_x // 2,
            :,
        ]
        spine_vol = spines_filtered_cp[
            z_min + volume_size_z // 2 : z_max + volume_size_z // 2,
            y_min + volume_size_y // 2 : y_max + volume_size_y // 2,
            x_min + volume_size_x // 2 : x_max + volume_size_x // 2,
        ]
        ...
        merge_masked_mip = cp.concatenate(
            (masked_image_mip, spine_mip, label_mip), axis=-1
        )
        ...
        merge_masked_slice = cp.concatenate(
            (masked_image_slice, spine_slice, label_slice), axis=-1
        )
...
def create_spine_arrays_in_blocks(
    image,
    labels,
    spines_filtered,
    table,
    ...
    for i in range(z_blocks):
        for j in range(y_blocks):
            for k in range(x_blocks):
                ...
                block_image = image[
                    padded_z_start:padded_z_end,
                    padded_y_start:padded_y_end,
                    padded_x_start:padded_x_end,
                ]
                block_spines_filtered = spines_filtered[
                    padded_z_start:padded_z_end,
                    padded_y_start:padded_y_end,
                    padded_x_start:padded_x_end,
                ]
                ...
                if len(block_table) > 0:
                    ...
                    (
                        ...
                        block_masked_slice,
                        block_vol,
                    ) = create_filtered_and_unfiltered_spine_arrays_cupy(
                        block_image,
                        block_spines_filtered,
                        block_labels,
                        block_table,
                        volume_size,
                        settings,
    ...
    imwrite(
        locations.arrays + "/Spine_MIPs_" + file,
        mip_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
    ...
    imwrite(
        locations.arrays + "/Spine_slices_" + file,
        slice_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
    ...
    imwrite(
        locations.arrays + "/Spine_masked_MIPs_" + file,
        masked_mip_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
    ...
    imwrite(
        locations.arrays + "/Spine_masked_slices_" + file,
        masked_slice_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
    ...
    imwrite(
        locations.arrays + "/Spine_vols_" + file,
...
def create_spine_arrays_in_blocks_4d(
    image,
    labels_filtered,
    table,
    volume_size,
    ...
    for i in range(z_blocks):
        for j in range(y_blocks):
            for k in range(x_blocks):
                ...
                block_labels_filtered = labels_filtered[
                    padded_z_start:padded_z_end,
                    padded_y_start:padded_y_end,
                    padded_x_start:padded_x_end,
                ]
                ...
                if len(block_table) > 0:
                    ...
                    (
                        block_mip,
                        block_slice_z,
                        block_mip_label_filtered,
                        block_slice_before_mip,
                    ) = create_filtered_and_unfiltered_spine_arrays_cupy(
                        block_image,
                        block_labels_filtered,
                        block_table,
                        volume_size,
                        settings,
                        locations,
    ...
    imwrite(
        locations.arrays + "/Spines_MIPs_" + file,
        mip_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
    ...
    imwrite(
        locations.arrays + "/Spines_Slices_" + file,
        slice_z_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
    ...
    imwrite(
        locations.arrays + "/Masked_Spines_MIPs_" + file,
        mip_label_filtered_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
    ...
    imwrite(
        locations.arrays + "/Masked_Spines_Slices_" + file,
        slice_before_mip_array.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
...
def create_MIP_spine_arrays_in_blocks_4d(
    image,
    y_locs,
    x_locs,
    volume_size,
        ...
        for t in range(padded_image.shape[0]):
            if np.isnan(yrow[t]):
                ...
                region = padded_image[
                    t,
                    :,
                    y_coord : y_coord + volume_size,
                    x_coord : x_coord + volume_size,
                ]
    ...
    imwrite(
        locations.input_dir + "/Registered/Isolated_spines_4D.tif",
        final_4d.astype(np.uint16),
        compression="zlib",
        compressionargs=1,
        imagej=True,
        photometric="minisblack",
...
def spine_measurementsV2(
    ...
    for ch in range(image.shape[1] - 1):
        ...
        # rename mean intensity
        table.rename(
            columns={"mean_intensity": "C" + str(ch + 2) + "_mean_int"}, inplace=True
        )
        table.rename(
            columns={"max_intensity": "C" + str(ch + 2) + "_max_int"}, inplace=True
        )
    ...
    # logger.info(f"  image labels before filter = {np.max(labels)}.")
    # integrated_density
    filtered_table["C1_int_density"] = (
        filtered_table["spine_vol"] * filtered_table["C1_mean_int"]
    )

    # measure remaining channels
    for ch in range(image.shape[1] - 1):
        filtered_table["C" + str(ch + 2) + "_int_density"] = (
            filtered_table["spine_vol"]
            * filtered_table["C" + str(ch + 2) + "_mean_int"]
        )
...
def get_bounding_box_cupy(mask, margin, shape, settings):
    ...
    z_min, z_max = (
        int(max(z.min().item() - margin_z, 0)),
        int(min(z.max().item() + margin_z + 1, shape[0])),
    )
    y_min, y_max = (
        int(max(y.min().item() - margin_y, 0)),
        int(min(y.max().item() + margin_y + 1, shape[1])),
    )
    x_min, x_max = (
        int(max(x.min().item() - margin_x, 0)),
        int(min(x.max().item() + margin_x + 1, shape[2])),
    )
...
def extract_subvolumes_GPU_batch(labeled_array, labels, padding=5):
    ...
    # Get binary masks for all labels in one batch operation
    binary_masks = (
        labeled_array[:, :, :, None] == labels[None, None, None, :]
    )  # Shape: (z, y, x, n_labels)
    ...
    for idx, label in enumerate(labels):
        ...
        start = cp.maximum(cp.min(coords, axis=0) - padding, 0)
            ...
            cp.max(coords, axis=0) + padding + 1, cp.array(labeled_array.shape)
        )

        # Extract the subvolume for the current label
        subvolume = (
            labeled_array[start[0] : end[0], start[1] : end[1], start[2] : end[2]]
            == label
        )
...
def spine_neck_analysis_gpu_batch(subvolumes, logger, scaling):
    ...
    for cp_subvolume in cp_subvolumes:
        ...
        length, min_width, max_width, mean_width, skeleton_result = (
            mesh_neck_width_and_length(cp_subvolume, verts, logger, scaling)
        )
...
def second_pass_annotation(
    ...
    avg_size = (
        8
        / settings.refinement_Z
        * 8
        / settings.refinement_XY
        * 8
        / settings.refinement_XY
    )  # spine or neck
    ...
    cp_multi_channel = cp.stack(
        [
            cp.asarray(head_labels_vol + neck_labels_vol),
            cp.asarray(neck_labels_vol > 0),
            cp.asarray(dendrite_vol > 0),
            cp.asarray(neuron_vol),
        ],
        axis=-1,
    )
    ...
    for i in range(0, len(labels), batch_size):
        ...
        # Extract subvolumes
        sub_volumes, start_coords = extract_subvolumes_mulitchannel_GPU_batch_2ndpass(
            cp_multi_channel, batch_labels
        )
        ...
        for subvol, label in zip(sub_volumes, batch_labels):
            # save as tif and resahpe for imageJ
            imwrite_filename = os.path.join(
                locations.nnUnet_2nd_pass, f"subvol_{10000 + label}_0000.tif"
            )
            # imageJ ZCYX - currently ZYXC so fix
            ...
            imwrite(
                imwrite_filename,
                subvol[:, :, :, 3].get().transpose(0, 1, 2).astype(np.uint16),
                compression="zlib",
                compressionargs=1,
                imagej=True,
                photometric="minisblack",
        ...
        return_code = sr.run_nnunet_predict(
        ...
        cp_multi_channel = insert_subvolumes(
            cp_multi_channel, multi_channel_subvolumes, start_coords, labels
        )
...
def insert_subvolumes(original_image, processed_subvolumes, start_coords, labels):
    ...
    for subvolume, start, label in zip(processed_subvolumes, start_coords, labels):
        ...
        label_mask = (
            original_image[start[0] : end[0], start[1] : end[1], start[2] : end[2], 0]
            == label
        )

        # Update the modified mask
        modified_mask[start[0] : end[0], start[1] : end[1], start[2] : end[2]] |= (
            label_mask
        )
        ...
        original_image[start[0] : end[0], start[1] : end[1], start[2] : end[2], 0] = (
            cp.where(
                ...
                0,
                original_image[
                    start[0] : end[0], start[1] : end[1], start[2] : end[2], 0
                ],
            )
        )
        original_image[start[0] : end[0], start[1] : end[1], start[2] : end[2], 1] = (
            cp.where(
                label_mask,
                0,
                original_image[
                    start[0] : end[0], start[1] : end[1], start[2] : end[2], 1
                ],
            )
        )
        ...
        original_image[start[0] : end[0], start[1] : end[1], start[2] : end[2], 0] += (
            cp.where(subvolume_mask[..., 0], subvolume[..., 0], 0)
        )
        original_image[start[0] : end[0], start[1] : end[1], start[2] : end[2], 1] += (
            cp.where(subvolume_mask[..., 2], subvolume[..., 2], 0)
        )
        ...
        original_image[start[0] : end[0], start[1] : end[1], start[2] : end[2], 2] = (
            cp.logical_or(
                original_image[
                    start[0] : end[0], start[1] : end[1], start[2] : end[2], 2
                ],
                subvolume[..., 1] > 0,
    ...
    if namespace is not None:
        ...
        for var_name in keys_to_delete:
            if logger is not None:
                ...
                logger.info(
                    f"[flush] Deleting large array '{var_name}' (~{size_mb:.2f} MB)."
                )
