...
def validate_analysis(labels1, labels2, settings, locations, logger):
    ...
    for file in range(len(analysis_files)):
        ...
        dendrites = np.isin(dendrite_labels, np.nonzero(large_dendrites)[0] + 1).astype(
            bool
        )
        ...
        logger.info(
            f"  {filt_dendrites} of {num_detected} detected dendrites larger than minimum volume threshold of {settings.min_dendrite_vol} voxels"
        )
        ...
        spine_labels = imgan.spine_detection(
        ...
        # Measurements
        spine_table, gt_spines = imgan.spine_measurementsV2(
            dendrites,
            spine_labels,
            1,
            max_label,
        ...
        # Measurements
        spine_table, output_spines = imgan.spine_measurementsV2(
            dendrites,
            spine_labels,
            1,
            max_label,
        ...
        # Measurements
        comp_spine_table = spine_comparison(
            gt_spines,
            output_spines,
            settings.neuron_spine_size,
            comp_spine_table,
        ...
        comp_spine_table.insert(
            15, "total_spine_vol_difference", gt_spine_volume - output_spine_volume
        )
        ...
        comp_spine_table.insert(
            18,
            "dendrite_length_difference",
            gt_dendrite_length - output_dendrite_length,
        )
        ...
        comp_spine_table.insert(
            22, "dendrite_vol_difference", gt_dendrite_volume - output_dendrite_volume
        )

        # append to summary
        # Append to the overall summary DataFrame
        comp_spine_summary = pd.concat(
...
def spine_comparison(gt, output, sizes, spine_table, logger):
    ...
    filtered_table = gt_table[
        (gt_table["area"] > volume_min) & (gt_table["area"] < volume_max)
    ]
    ...
    filtered_table = output_table[
        (output_table["area"] > volume_min) & (output_table["area"] < volume_max)
    ]
