...
class EpochLoggingCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        logs = logs or {}
        self.logger.info(
            f"Epoch {epoch + 1} - loss: {logs.get('loss'):.4f} - val_loss: {logs.get('val_loss'):.4f}"
        )
...
def _aug_complete(src_dir, tgt_dir, n_src, n_rot):
    ...
    return len(src_files) == len(tgt_files) == expected and src_files == tgt_files


def _bad_volumes(folder):
    bad = []
    for f in os.listdir(folder):
        if f.lower().endswith(".tif"):
            arr = imread(os.path.join(folder, f))
            z = arr.shape[0]
            if z < 8 or z % 8:
...
def _pad_to_mult(img, mult=8):
    z = img.shape[0]
    pad = mult - z % mult if z % mult else 0
    if z < mult:
        pad += mult - z
    if pad == 0:
        return img
    pre = pad // 2
    post = pad - pre
    return np.pad(img, ((pre, post), (0, 0), (0, 0)), mode="reflect")


def _pad_dir(path, mult=8, logger=None):
    for f in os.listdir(path):
        if f.lower().endswith(".tif"):
            p = os.path.join(path, f)
            arr = imread(p)
            z0 = arr.shape[0]
            arr = _pad_to_mult(arr, mult)
            if arr.shape[0] != z0:
                imwrite(p, arr, imagej=True, metadata={"axes": "ZYX"})
                logger.info(f"    Padded {f}: {z0} - {arr.shape[0]} slices")
...
def train_care(
    ...
    if image_type == "2D":
        ...
        axes = "ZYX"
    if axes == "ZYX":
        bad = _bad_volumes(low_snr_dir) + _bad_volumes(high_snr_dir)
        if bad:
            for fname, shape in bad:
                logger.error(f"    Volume fails Z check: {fname}  shape={shape}")
            logger.error("  Padding data...")
            _pad_dir(low_snr_dir, 8, logger)
            _pad_dir(high_snr_dir, 8, logger)
    ...
    if augmentation:
        ...
        n_original_tif = len(
            [
                f
                for f in os.listdir(os.path.join(inputdir, "lowSNR"))
                if f.lower().endswith(".tif")
            ]
        )

        if _aug_complete(
            output_augmented_dir, output_high_snr_dir, n_original_tif, len(degrees_list)
        ):
            logger.info(
                "CARE augmentations complete – skipping regeneration. "
                "Delete the augmentation folders to recreate."
            )
        else:
            # clean partial results
            for d in (output_augmented_dir, output_high_snr_dir):
                if os.path.isdir(d):
                    shutil.rmtree(d, ignore_errors=True)
                os.makedirs(d, exist_ok=True)

            logger.info("Generating CARE augmentations…")
            augment_images_v1(
                os.path.join(inputdir, "lowSNR"),
    ...
    (X, Y), (X_val, Y_val), axes = load_training_data(
        "CAREdata/my_training_data.npz", validation_split=pct_validation, verbose=True
    )
    ...
    logger.info(
        f"axes: {axes}, n_channel_in: {n_channel_in}, n_channel_out: {n_channel_out}, "
    ...
    config = Config(
        ...
        train_learning_rate=initial_learning,
    )
...
def generate_nnunet_json(input_folder, use_ignore=False):
    ...
    num_channels = len(
        [f for f in os.listdir(imagesTr_path) if f.endswith(".tif")]
...
def run_process_with_logging(cmd, logger, env=None):
    ...
    stdout_thread = threading.Thread(
        target=log_output, args=(process.stdout, logger, logger.info)
    )
    stderr_thread = threading.Thread(
        target=log_output, args=(process.stderr, logger, logger.error)
    )
    stdout_thread.start()
    stderr_thread.start()
