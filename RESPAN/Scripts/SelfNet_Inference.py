...
def reslice_old_selfnet(img, position, x_res, z_res):
    ...
    if position == "xz":
        ...
        for i in range(y):
            scale_img[i] = cv2.resize(
                reslice_img[i], (x, round(z * scale)), interpolation=cv2.INTER_CUBIC
            )
        ...
        for i in range(x):
            scale_img[i] = cv2.resize(
                reslice_img[i], (y, round(z * scale)), interpolation=cv2.INTER_CUBIC
            )
...
def output_img(deblur_net, device, min_v, max_v, write_stack, raw_img):
    ...
    for ii in range(idx):
        with torch.no_grad():
            test_tensor = input_tensor[ii * batch_size : (ii + 1) * batch_size].to(
                device
            )
...
class myThread(threading.Thread):
    def __init__(
        self, threadID, name, deblur_net, device, min_v, max_v, write_stack, raw_img
    ):
...
def upsample_block(raw_img, x_res, z_res, deblur_netA, deblur_netB, min_v, max_v):
    ...
    thread1 = myThread(
        1, "thread:1", deblur_netA, device1, min_v, max_v, out_xz_img, xz_img
    )
    thread2 = myThread(
        2, "thread:2", deblur_netB, device2, min_v, max_v, out_yz_img, yz_img
    )
...
parser.add_argument(
    "--neuron_ch", type=int, default=0, help="the channel for inference"
)
...
parser.add_argument(
    "--scale", type=float, default=0.21, help="The resolution scaling factor"
)
...
for filename in os.listdir(input_dir):
    ...
    if filename.endswith(".tif") or filename.endswith(".tiff"):
        ...
        fusion_stack = upsample_block(
            image, scale, z_step, deblur_net_A, deblur_net_B, min_v, max_v
        )
