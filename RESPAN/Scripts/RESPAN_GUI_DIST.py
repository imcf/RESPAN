...
def find_python_executable(venv_dir: Path) -> Optional[Path]:
    """Return the python executable Path inside a virtual/conda env, or None if not found.

    This checks common locations on Windows and Unix-like systems and keeps
    the existing Windows-first behavior so current setups keep working.
    """
    candidates = [
        venv_dir / "python.exe",
        venv_dir / "python",
...
from PyQt5.QtGui import QColor, QFont, QIcon, QPainter, QPalette, QPixmap, QTextCursor
...
# Base Dir
APP_DIR = (
    Path(sys.executable).parent if FROZEN else Path(__file__).resolve().parent
)  # …\Scripts

if FROZEN:
    VENV_DIR = APP_DIR / "_internal" / "respan"
    # Pick the appropriate python executable for the embedded environment (cross-platform)
    PY_EXE = find_python_executable(VENV_DIR)
    if PY_EXE is None:
        raise RuntimeError(f"Embedded Python not found in {VENV_DIR}")
    ...
    # Find a python executable in the conda environment (works on Windows and Unix)
    PY_EXE = find_python_executable(VENV_DIR)
    if PY_EXE is None:
        # Fall back to the current interpreter if available (e.g., running under pixi)
        current_py = Path(sys.executable) if sys.executable else None
        if current_py and current_py.exists():
            PY_EXE = current_py
            print(
                f"Warning: Dev env '{DEV_ENV}' not found at {VENV_DIR}. Falling back to current interpreter: {PY_EXE}"
            )
        else:
            # Try to find python from PATH
            path_py = shutil.which("python3") or shutil.which("python")
            if path_py:
                PY_EXE = Path(path_py)
                print(
                    f"Warning: Dev env '{DEV_ENV}' not found at {VENV_DIR}. Falling back to python found on PATH: {PY_EXE}"
                )
            else:
                raise RuntimeError(
                    f"Dev env '{DEV_ENV}' not found at {VENV_DIR}. No python executable detected."
                )

    VENV_DIR = conda_root / "envs" / DEV_ENV
    INSTALL_DIR = Path(r"C:\Users\Luke_H") if os.name == "nt" else Path.home()
...
# Determine script directory and nnUNet script names cross-platform
script_dir_name = "Scripts" if os.name == "nt" else "bin"
nnunet_predict_bat = (
    VENV_DIR
    / script_dir_name
    / ("nnUNetv2_predict.bat" if os.name == "nt" else "nnUNetv2_predict")
)
nnunet_plan_bat = (
    VENV_DIR
    / script_dir_name
    / (
        "nnUNetv2_plan_and_preprocess.bat"
        if os.name == "nt"
        else "nnUNetv2_plan_and_preprocess"
    )
)
nnunet_train_bat = (
    VENV_DIR
    / script_dir_name
    / ("nnUNetv2_train.bat" if os.name == "nt" else "nnUNetv2_train")
)

# Warn if expected nnUNet scripts aren't present in the env (helps debugging cross-platform setups)
if not nnunet_predict_bat.exists():
    print(f"Warning: nnUNet predict script not found at {nnunet_predict_bat}")
if not nnunet_plan_bat.exists():
    print(f"Warning: nnUNet plan script not found at {nnunet_plan_bat}")
if not nnunet_train_bat.exists():
    print(f"Warning: nnUNet train script not found at {nnunet_train_bat}")
...
# Ensure env-local CUDA runtime libraries are visible when running inside a conda/mamba/pixi env
import ctypes


def _add_env_lib_to_ld_library_path(py_exe_path):
    """Add the environment lib dir (e.g., $CONDA_PREFIX/lib) to LD_LIBRARY_PATH.

    This helps TensorFlow find libcudart/libcudnn when they are installed inside the active env.
    """
    conda_prefix = os.environ.get("CONDA_PREFIX")
    if not conda_prefix and py_exe_path:
        # Derive prefix from python executable: /.../envs/default/bin/python -> /.../envs/default
        conda_prefix = str(Path(py_exe_path).resolve().parent.parent)
    ...
    if libdir.exists():
        ...
        paths = ld.split(os.pathsep) if ld else []
        if str(libdir) not in paths:
            os.environ["LD_LIBRARY_PATH"] = str(libdir) + (
                os.pathsep + ld if ld else ""
            )
            print(f"Added environment lib dir to LD_LIBRARY_PATH: {libdir}")
    else:
        print(f"Environment lib dir not present: {libdir}")


def _check_cuda_libraries(py_exe_path):
    """Check for presence of key CUDA runtime libraries and give recommendations."""
    conda_prefix = os.environ.get("CONDA_PREFIX")
    if not conda_prefix and py_exe_path:
        conda_prefix = str(Path(py_exe_path).resolve().parent.parent)
    libdir = Path(conda_prefix) / "lib" if conda_prefix else None

    required = ["libcudart.so.11", "libcublas.so.11", "libcudnn.so.8"]
    found = {}
    for r in required:
        found[r] = False
        # Search in libdir first
        if libdir and libdir.exists():
            for p in libdir.glob(f"{r}*"):
                found[r] = True
                break
        # Then search in system ld cache
        if not found[r]:
            try:
                _ = ctypes.CDLL(r)
                found[r] = True
    ...
    for r, ok in found.items():
        if ok:
            print(f"Found CUDA library: {r}")
        else:
            print(
                f"Missing CUDA library: {r}.\n  Recommend: install matching cudatoolkit + cudnn into the active env (e.g., `pixi run mamba install -n default -c nvidia -c conda-forge cudatoolkit=11.2 cudnn=8.* -y`) or ensure system CUDA 11.x is available and visible via LD_LIBRARY_PATH."
            )


# Run helper to try to make env-local CUDA libs visible
_add_env_lib_to_ld_library_path(PY_EXE)
_check_cuda_libraries(PY_EXE)


def _preload_cuda_libraries(py_exe_path):
    """Attempt to load critical CUDA libraries into the process with RTLD_GLOBAL.

    This helps make them available for extensions (like TensorFlow) that are
    imported later in the same process and may depend on these shared objects.
    """
    conda_prefix = os.environ.get("CONDA_PREFIX")
    if not conda_prefix and py_exe_path:
        conda_prefix = str(Path(py_exe_path).resolve().parent.parent)
    ...
    for name in candidates:
        loaded = False
        # try env libdir first
        if libdir and libdir.exists():
            for p in libdir.glob(f"{name}*"):
                try:
                    ctypes.CDLL(str(p), mode=ctypes.RTLD_GLOBAL)
                    print(f"Preloaded CUDA library: {p}")
                    loaded = True
                    break
                except Exception as e:
                    print(f"Failed to preload {p}: {e}")
        if not loaded:
            # try loading by SONAME (system paths)
            try:
                ctypes.CDLL(name, mode=ctypes.RTLD_GLOBAL)
                print(f"Preloaded CUDA library by SONAME: {name}")
                loaded = True
            except Exception as e:
...
# Attempt to preload libs so TensorFlow can import them reliably
_preload_cuda_libraries(PY_EXE)
###
...
## suppress windows-specific startup info with subprocess on Windows only
if os.name == "nt":
    # Windows-only STARTUPINFO to hide console windows when launching subprocesses
    startupinfo = subprocess.STARTUPINFO()
    startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
    startupinfo.wShowWindow = subprocess.SW_HIDE
else:
    # On Unix-like systems, subprocess.STARTUPINFO is not available
    startupinfo = None
...
def run_external_script_prev(
    script_path: str, conda_prefix: str, env_name: str, args: dict[str, str], logger
):

    env_path = os.path.join(conda_prefix, "envs", env_name)
    # Search for a python executable in the environment (cross-platform)
    python_candidates = [
    ...
    if python_exe is None:
        raise RuntimeError(
            f"Python executable not found in environment '{env_name}' at {env_path}"
        )
...
class EnvSetup(QThread):
            ...
            if FROZEN:
                ...
                if VENV_DIR.exists() and PY_EXE.exists():
                    ...
                    self.finished_signal.emit(
                        True, f"RESPAN environment ready at {VENV_DIR}"
                    )
                ...
                if VENV_DIR.exists() and PY_EXE.exists():
                    self.finished_signal.emit(
                        True, f"RESPAN environment found at {VENV_DIR}"
                    )
                    self.finished_signal.emit(True, "\nInitialization complete.")
                else:
                    self.finished_signal.emit(
                        False, f"Development environment '{DEV_ENV}' not found"
                    )
    ...
    def _ensure_nnunet_installed(self):
        ...
        self.logger.info(
            f"First time running RESPAN, please allow a few minutes to complete installation..."
        )
...
class RESPANAnalysis(QWidget):
    ...
    def initUI(self):
        ...
        self.info_label = QLabel(
            "*RESPAN is optimized for batch processing. Images need to be organized in subfolders \nof the selected data directory. E.g. a subfolder for each animal or condition."
        )
        self.info_label2 = QLabel(
            "Ensure the correct Analysis_Settings.yaml file is included in each subfolder."
        )
        self.line2 = QFrame()
        ...
        model_options = [
            "Spines, Dendrites, and Soma (original)",
            "Dendrites and Soma Only",
            "Spines, Necks, Dendrites, and Soma",
            "Spines, Necks, Dendrites, Soma, and Axons (2025)",
        ]
        ...
        self.float_label_1 = QLabel(
            "Minimum dendrite volume (µm<sup>3</sup>, dendrites smaller than this will be ignored):"
        )
        ...
        self.float_label_2 = QLabel(
            "Spine volume filter (min, max volume in µm<sup>3</sup>):"
        )
        ...
        self.float_label_3 = QLabel(
            "Spine distance filter (max distance from dendrite in µm):"
        )
        ...
        options_tracking.setStyleSheet(
            "QGroupBox::title {"
            "subcontrol-origin: margin;"
            "subcontrol-position: top left;"
            "padding: 2px;"
        ...
        self.reg_label = QLabel(
            "Select registration method for aligning volumes across time:"
        )
        ...
        self.save_validation = QCheckBox(
            "Save validation data (2D MIPs for confirming dendrites and spines)"
        )
        ...
        self.save_intermediate = QCheckBox(
            "Additional data and logging (3D volumes of dendrites, spines, meshes,and data for detailed inspection)"
        )
        self.save_intermediate.setChecked(False)
        self.nnUNet_patching = QCheckBox(
            "Enable patching for nnUNet (recommended for datasets >1GB)"
        )
        ...
        self.image_restore_opt = QCheckBox(
            "Use image restoration (*requires trained CARE models)"
        )
        self.image_restore_opt.setChecked(False)
        self.axial_restore_opt = QCheckBox(
            "Use axial restoration (*requires trained SelfNet model)"
        )
        self.axial_restore_opt.setChecked(False)

        self.swc_gen = QCheckBox(
            "Generate SWC files for dendrite/neuron (via Vaa3D APP2 plugin)"
        )
                        ...
                        try:
                            ...
                            if device_count > 0:
                                for i in range(device_count):
                                    ...
                                    self.log_display.append(
                                        f"     Free Memory: {memory_info.free / 1024**2:.2f} MB"
                                    )
                                    self.log_display.append(
                                        f"     TensorFlow version: {tf.__version__}"
                                    )
                                    self.log_display.append(
                                        f"     CUDA version: {tf.sysconfig.get_build_info()['cuda_version']}"
                                    )
                                    self.log_display.append(
                                        f"     cuDNN version: {tf.sysconfig.get_build_info()['cudnn_version']}"
                                    )
                                    self.log_display.append(
                                        f"     CUDA Path: {os.environ.get('CUDA_PATH', 'Not set')}\n"
                                    )
                                    # self.log_display.append(f"Used Memory: {memory_info.used / 1024 ** 2:.2f} MB")
                        except Exception as e:
                            self.log_display.append(
                                f"Failed to retrieve GPU information: {str(e)}"
                            )
                    except RuntimeError as e:
                        self.log_display.append(f"Could not set GPU: {str(e)}")
                    except Exception as e:
                        self.log_display.append(
                            f"Error retrieving GPU details: {str(e)}"
                        )
            else:
    ...
    def validate_run_inputs(self, RESPAN, directory, model_dir):
        """Validate required run inputs and return (ok, message)."""
        msgs = []
        if global_GUI_app == False:
            if not RESPAN or RESPAN.strip() == "" or RESPAN.startswith("No RESPAN"):
                msgs.append("No RESPAN directory selected.")
        if (
            not directory
            or directory.strip() == ""
            or directory.startswith("No directory selected")
        ):
            msgs.append("No data directory selected.")
        elif not os.path.exists(directory):
            msgs.append(f"Data directory does not exist: {directory}")
        else:
            subfolders = [
                d
            ...
            if len(subfolders) == 0:
                msgs.append(
                    "Selected data directory contains no subfolders. Please ensure datasets are stored in subfolders."
                )
        if (
            not model_dir
            or model_dir.strip() == ""
            or model_dir.startswith("No directory selected")
        ):
            msgs.append("No model directory selected.")
        elif not os.path.exists(model_dir):
            msgs.append(f"Model directory does not exist: {model_dir}")
    ...
    def run_function(self):
        ...
        # Wrap run flow in try/except to capture and display unexpected exceptions
        try:
            if global_GUI_app == False:
            ...
            model_dir = self.model_directory_label.text().replace(
                "Selected directory: ", ""
            )
            ...
            # Full validation step before starting the worker
            ok, vmsg = self.validate_run_inputs(RESPAN, directory, model_dir)
            if not ok:
                self.logger.info(vmsg)
                QMessageBox.critical(self, "Validation Failed", vmsg)
                self.progress.setVisible(False)
                return
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", "Invalid input for minimum dendrite volume."
            )
            self.progress.setVisible(False)
            return
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", "Invalid input for spine distance to dendrite."
            )
            self.progress.setVisible(False)
            return
        ...
        # Preflight check for nnU-Net preprocessed data for the selected model
        try:
            model_path_norm = os.path.normpath(model_dir)
            ...
            if dataset_id is not None:
                pp_env = os.environ.get("nnUNet_preprocessed")
                if pp_env and os.path.exists(
                    os.path.join(pp_env, f"Dataset{dataset_id}")
                ):
                    preprocessed_found = True
                else:
                    p = Path(model_dir).parent
                    for _ in range(3):
                        candidate = p / "nnunet_preprocessed" / f"Dataset{dataset_id}"
                        if candidate.exists():
                            preprocessed_found = True
                            break
                        p = p.parent

            if dataset_id and not preprocessed_found:
                msg = QMessageBox(self)
                msg.setIcon(QMessageBox.Warning)
                msg.setWindowTitle("nnU-Net preprocessed data missing")
                msg.setText(
                    f"Preprocessed nnU-Net data for dataset {dataset_id} was not found.\n\n"
                    "Prediction requires preprocessed files. You can create them using ``nnUNetv2_plan_and_preprocess`` or set the environment variable ``nnUNet_preprocessed`` to point to a folder containing the Dataset.\n\n"
                    "What would you like to do?"
                )
                show_inst = msg.addButton("Show instructions", QMessageBox.ActionRole)
                cont_btn = msg.addButton("Continue anyway", QMessageBox.DestructiveRole)
                cancel_btn = msg.addButton("Cancel", QMessageBox.RejectRole)
                msg.exec_()
                clicked = msg.clickedButton()
                if clicked == show_inst:
                    instr = (
                        "To create preprocessed data:\n\n"
                        "1) In the nnU-Net environment run:\n"
                        f"   nnUNetv2_plan_and_preprocess -d {dataset_id} --verify_dataset_integrity\n\n"
                        "2) Set env vars before launching RESPAN:\n"
                        "   export nnUNet_preprocessed=/path/to/nnunet_preprocessed\n"
                        "   export nnUNet_raw=/path/to/nnunet_raw\n\n"
                        "Alternatively, run the Train nnU-Net workflow in RESPAN's Train tab to perform plan+preprocess+train.\n"
                    )
                    QMessageBox.information(
                        self, "nnU-Net Preprocess Instructions", instr
                    )
                    try:
                        QApplication.clipboard().setText(instr)
                        self.logger.info("Preprocess instructions copied to clipboard.")
                    except Exception:
                        pass
                    self.progress.setVisible(False)
                    return
                    ...
                    self.progress.setVisible(False)
                    return
                # else user chose Continue anyway -> proceed
        except Exception as e:
            self.logger.info(f"Preflight check failed: {e}")
        ...
        # Log that we're constructing the worker
        self.logger.info("Preparing AnalysisWorker with provided parameters...")
        ...
        try:
            self.worker.started.connect(
                lambda: self.logger.info("AnalysisWorker thread started.")
            )
        except Exception:
            # In case the signal isn't available, ignore
            pass
        ...
        try:
            ...
            self.logger.error(f"Failed to start AnalysisWorker: {e}\n{tb}")
            QMessageBox.critical(self, "Error", f"Failed to start worker: {e}")
            self.progress.setVisible(False)
            return

        # Safety timer: verify worker started within 2s
        QTimer.singleShot(2000, lambda: self._check_worker_started())

    def _check_worker_started(self):
        if not hasattr(self, "worker") or not self.worker.isRunning():
            self.logger.error("Worker failed to start or exited immediately.")
            QMessageBox.critical(
                self,
                "Error",
                "Worker failed to start or exited immediately. Check logs.",
            )
            self.progress.setVisible(False)
            return
...
class AnalysisWorker(QThread):
    ...
    def run(self):
        ...
        try:
            self.logger.info("AnalysisWorker: run() starting...")
            self.logger.info(f"  RESPAN: {self.RESPAN}, directory: {self.directory}")

            if global_GUI_app == False:
                sys.path.append(self.RESPAN)

            from RESPAN.Environment import imgan, io, main, sr, strk, timer

            main.check_gpu()

            if self.directory == "No data directory selected." or not self.directory:
                msg = "No directory has been selected. Please select a valid data directory first."
            ...
            for subfolder in os.listdir(self.directory):
                ...
                if os.path.isdir(subfolder_path):
                    ...
                    settings.neuron_spine_size = [
                        ...
                        for x in self.spine_vol
                    ]
                    settings.neuron_spine_dist = round(
                        self.spine_dist / (settings.input_resXY), 2
                    )
                    ...
                    settings.nnunet_stride = tuple(
                        int(p * 0.9) for p in settings.nnunet_patch_size
                    )
                    ...
                    self.logger.info(
                        f"  Image resolution: {settings.input_resXY}µm XY, {settings.input_resZ}µm Z"
                    )
                    self.logger.info(f"  Model used: {settings.neuron_seg_model_path}")
                    self.logger.info(f"  Model type: {self.model_type}")
                    self.logger.info(
                        f"  Model resolution: {settings.model_resXY}µm XY, {settings.model_resZ}µm Z"
                    )
                    self.logger.info(
                    ...
                    self.logger.info(
                        f"  Dendrite volume set to: {self.min_dendrite_vol} µm, {settings.min_dendrite_vol} voxels"
                    )
                    self.logger.info(
                        f"  Spine volume set to: {self.spine_vol[0]} to {self.spine_vol[1]} µm³, {settings.neuron_spine_size[0]} to {settings.neuron_spine_size[1]} voxels"
                    )
                    self.logger.info(
                        f"  Spine distance filter set to: {self.spine_dist} µm, {settings.neuron_spine_dist} pixels"
                    )
                    self.logger.info(f"  Analysis method: {self.analysis_method}")
                    # self.logger.info(f" Second pass enabled: {self.second_pass}")
                    self.logger.info(
                        f"  GPU block size set to: {settings.GPU_block_size[0]},{settings.GPU_block_size[1]},{settings.GPU_block_size[1]}"
                    )
                    self.logger.info(
                        f"  Tracking set to: {settings.Track}, using {settings.reg_method} registration."
                    )
                    # self.logger.info(f"  Histogram matching set to: {settings.HistMatch}")
                    self.logger.info(
                        f"  Image restoration set to {self.image_restore} and axial restoration set to {self.axial_restore}"
                    )
                    self.logger.info(f"  Neck generation {self.neck_generation}")
                    self.logger.info(
                        f"  Dask parallelization set to: {self.dask_enabled}"
                    )
                    # log dask block and halo
                    if self.dask_enabled:
                        self.logger.info(
                            f"  Dask block size set to: {settings.dask_block[0]},{settings.dask_block[1]},{settings.dask_block[2]}"
                        )
                        self.logger.info(
                            f"  Dask halo size set to: {settings.dask_halo[0]},{settings.dask_halo[1]},{settings.dask_halo[2]}"
                        )
                    if settings.patch_for_nnunet:
                        self.logger.info(
                            f"  nnUNet patching enabled.Patch size set to: {settings.nnunet_patch_size[0]},{settings.nnunet_patch_size[1]},{settings.nnunet_patch_size[2]}"
                        )
                    else:
                    ...
                    if os.path.exists(f"{global_env_path}/{global_respan_env}/"):
                        ...
                        if settings.Track == True and log == 0:
                            ...
                            imgan.analyze_spines_4D(
                                settings, locations, log, self.logger
                            )
                            self.logger.info(
                        ...
                        # nnUNet env not found at configured path — check alternatives (PATH or module)
                        nnunet_env_dir = os.path.join(
                            global_env_path, global_respan_env
                        )
                        path_candidate = shutil.which(
                            "nnUNetv2_predict"
                        ) or shutil.which("nnunetv2_predict")
                        try:
                            import importlib.util

                            module_ok = importlib.util.find_spec(
                                "nnunetv2_predict"
                            ) or importlib.util.find_spec("nnunetv2")
                        except Exception:
                            module_ok = None

                        if path_candidate or module_ok:
                            self.logger.info(
                                "nnUNet not found in configured env, but found on PATH or as a module — proceeding using current environment."
                            )
                            self.logger.info(f" PATH candidate: {path_candidate}")
                            # proceed with processing — run_nnunet_predict will use PATH or python -m fallback
                            log = sr.restore_and_segment(
                                settings, locations, self.logger
                            )

                            # Post-processing (was only executed when nnUNet env existed).
                            if settings.Track == False and log == 0:
                                imgan.analyze_spines(
                                    settings, locations, log, self.logger
                                )
                            ...
                            if log != 0:
                                self.logger.info(
                                    "Error with restoration and segmentation."
                                )
                                self.logger.info(
                                    "-----------------------------------------------------------------------------------------------------"
                                )
                            ...
                            self.logger.info(
                                "Error: nnUNet environment not created or set correctly."
                            )
                            self.logger.info(
                                " Path set to:"
                                + global_env_path
...
class RESPANValidation(QWidget):
    def __init__(self):
        ...
        self.neuron_channel_input = QLineEdit("1")
        self.float_label_1 = QLabel(
            "Minimum dendrite size in µm (dendrites smaller than this will be ignored):"
        )
        self.float_input_1 = QLineEdit("15")
        self.float_label_2 = QLabel(
            "Spine volume filter (min, max volume in µm<sup>3</sup>):"
    ...
    def get_outputdir(self):
        datadir = QFileDialog.getExistingDirectory(
            self, "Select data analysis output directory"
        )
        if datadir:
            self.directory_label.setText(f"Selected directory: {datadir}")
    ...
    @pyqtSlot()
    def get_gtdir(self):
        gtdir = QFileDialog.getExistingDirectory(
            self, "Select ground truth data directory"
        )
        if gtdir:
    ...
    def run_function(self):
        ...
        ground_truth = self.ground_truth_dir_label.text().replace(
            "Selected directory: ", ""
        )
    ...
    def run(self):
        ...
        try:
            ...
            log_path = (
                ...
                + "RESPAN_Validation_Log"
                + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                + ".log"
            )
            self.logger.set_log_file(
                log_path
            )  # Note: we're not passing a log_display, because it's not thread-safe
            ...
            self.logger.info(
                f" Image resolution: {settings.input_resXY}um XY, {settings.input_resZ}um Z"
            )
            self.logger.info(
                f" Spine volume set to: {self.spine_vol[0]} to {self.spine_vol[1]} um<sup>3</sup>, {settings.neuron_spine_size[0]} to {settings.neuron_spine_size[1]} voxels."
            )
            self.logger.info(
                f" Spine distance filter set to: {self.spine_dist} um, {settings.neuron_spine_dist} pixels"
            )
            self.logger.info(
                f" GPU block size set to: {settings.GPU_block_size[0]},{settings.GPU_block_size[1]},{settings.GPU_block_size[1]}"
            )
...
class UNet(QWidget):
    def __init__(self):
        ...
        self.line.setFrameShadow(QFrame.Sunken)
        self.info_label2 = QLabel(
            "nnUNet input folder should contain imagesTr and labelsTr subfolders - refer to guide for more information."
        )
        self.line2 = QFrame()
        self.line2.setFrameShape(QFrame.HLine)
        self.line2.setFrameShadow(QFrame.Sunken)
    ...
    def get_outputdir(self):
        datadir = QFileDialog.getExistingDirectory(
            self, "Select model output directory"
        )
        if datadir:
            self.directory_label.setText(f"Selected directory: {datadir}")
    ...
    def run_function(self):
        ...
        self.worker = UNetWorker(
            RESPAN, input_dir, model_output, datasetID, ignore, self.logger
        )
        self.worker.task_done.connect(self.on_task_done)
        self.worker.start()
...
class UNetWorker(QThread):
    ...
    # def __init__(self, RESPAN, ground_truth, analysis_output, logger):
    def __init__(self, RESPAN, input_dir, model_output, datasetID, ignore, logger):
        super().__init__()
        self.RESPAN = RESPAN
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(global_env_path + "/" + global_respan_env + "/"):
                ...
                # Generate JSON file
                json_data = mt.generate_nnunet_json(
                    self.input_dir, use_ignore=self.ignore
                )
                ...
                self.logger.info(
                    " Displaying nnU-Net data information for confirmation:\n"
                    + json_str
                )
                ...
                self.logger.info(
                    "nnU-Net Training GUI | RESPAN Version: " + __version__
                )
                self.logger.info("Release Date: " + __date__ + "")
                self.logger.info("Created by: " + __author__ + "")
                self.logger.info("Department of Neurology, The Ohio State University")
                ...
                self.logger.info(
                    "Environment not found. Please check the path and environment name."
                )
...
class CARE(QWidget):
    def __init__(self):
        ...
        self.ground_truth_dir_label = QLabel("No training data directory selected.")
        self.ground_truth_dir_button = QPushButton(
            "Select select training data directory"
        )
        self.ground_truth_dir_button.clicked.connect(self.get_gtdir)
        self.info_labeltraing = QLabel(
            "Folder should contain subfolders lowSNR and highSNR3D contained matched datasets."
        )
        ...
        self.model_name_input.setText(
            "example_model_name_dendrites_60x_150nmXY_300nmZ_60epochs"
        )
    ...
    def run_function(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", "Invalid input for number of patches per image."
            )
            self.progress.setVisible(False)
            return
    ...
    def run(self):
        ...
        try:
            ...
            if not os.path.isdir(
                os.path.join(self.input_dir, "highSNR")
            ) or not os.path.isdir(os.path.join(self.input_dir, "lowSNR")):
                self.logger.info(
                ...
                self.logger.info(
                    f" Low SNR Data Directory: {os.path.join(self.input_dir, 'lowSNR')}"
                )
                self.logger.info(
                    f" Low SNR Data Directory: {os.path.join(self.input_dir, 'highSNR')}"
                )
                ...
                self.logger.info(
                    f" Number of Patches per Image: {self.num_patches_per_img}"
                )
...
class SelfNet(QWidget):
    def __init__(self):
        ...
        self.line.setFrameShadow(QFrame.Sunken)
        self.info_label2 = QLabel(
            "Self-Net input folder should contain at least one .tif image - refer to guide for more information."
        )
        self.line2 = QFrame()
        self.line2.setFrameShape(QFrame.HLine)
        self.line2.setFrameShadow(QFrame.Sunken)
        ...
        self.model_name_input.setFixedWidth(400)
        self.model_name_input.setText(
            "example_model_name_Self-Net_dendrites_60x_150nmXY_300nmZ"
        )
    ...
    def run_function(self):
        ...
        try:
            ...
            for dir_path, label in [
                (ground_truth, "training data directory"),
...
class SelfNetWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info(
                f" Using conda environment: {global_env_path}/{global_respan_env}"
            )
...
class About(QWidget):
    def __init__(self):
        ...
        respan_info = QGroupBox(
            "Restoration Enhanced Neuron and SPine Analysis (RESPAN)"
        )
