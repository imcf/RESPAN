...
def reslice(data, plane, scale_factor):
    ...
    # Efficient resizing
    resliced = resize(
        resliced, new_dimensions, order=3, mode="reflect", preserve_range=True
    )
...
def reslice_and_resize_torch(data, plane, scale_factor, device="cuda"):
    ...
    resized_data = F.interpolate(
        data, size=new_size, mode="trilinear", align_corners=False
    )
    ...
    resized_data = (
        (resized_data - original_min) / (original_max - original_min) * 65535
    ).astype(np.uint16)
...
def self_net_create_slices(input_dir, filename, scale):
    ...
    for index in range(p):
        ...
        downsampled_image = resize(
            image, (round(m * scale), n), order=3, mode="reflect", preserve_range=True
        )
        # Upsample the image back to original size
        upsampled_image = resize(
            downsampled_image, (m, n), order=3, mode="reflect", preserve_range=True
        )

        # Save the processed image
        imwrite(
            os.path.join(xy_lr_path, f"{index + 1}.tif"),
            upsampled_image.astype(image.dtype),
        )
...
def generate_training_data(
    input_dir, signal_intensity_threshold, xy_interval, xz_interval
):
...
def train(
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_set):
            ...
            loss_cycle_A = lambda_cycle * criterionL1(rec_A, A) + lambda_ssim * (
                1 - SSIM_loss(rec_A, A)
            )

            # Backward cycle loss || G_A(G_B(B)) - B||

            loss_cycle_B = lambda_cycle * criterionL1(rec_B, B) + lambda_ssim * (
                1 - SSIM_loss(rec_B, B)
            )

            loss_feedback = lambda_feedback * (
                criterionL1(fake_C1, C) + lambda_ssim * (1 - SSIM_loss(fake_C1, C))
            )
            ...
            if (i + 1) % 2 == 0:
                ...
                loss_D_A = backward_D_basic(
                    lambda_gan, netD_A, criterionGAN, B, fake_B, "lsgan", device
                )

                fake_A = fakeA_pool.query(fake_A)
                loss_D_B = backward_D_basic(
                    lambda_gan, netD_B, criterionGAN, A, fake_A, "lsgan", device
                )
            ...
            if (i + 1) % log_interval == 0:
                print(
                    "epochs: {} {}/{}  degradation modeling stage: loss_D:{:4f} , "
                    "loss_G: {:4f} loss_G_cycle:{:4f} loss_feedback:{:4f}".format(
                        epoch + 1,
                        (i + 1) * batch_size,
                        len(train_set.dataset),
                        Loss_D / (log_interval * lambda_gan),
                        Loss_G_GAN / (log_interval * lambda_gan),
                        Loss_G_cycle / (log_interval * lambda_cycle),
                        Loss_feedback / (log_interval),
                    ),
                    flush=True,
                )
            ...
            loss_deblur = lambda_deblur * criterionL1(fake_C2, C) + lambda_ssim * (
                1 - SSIM_loss(fake_C2, C)
            )
            ...
            if (i + 1) % log_interval == 0:
                print(
                    "epochs: {} {}/{}  deblurring stage :loss_deblur:{:4f}".format(
                        epoch + 1,
                        (i + 1) * batch_size,
                        len(train_set.dataset),
                        Loss_deblur / (log_interval * lambda_deblur),
                    ),
                    flush=True,
                )
...
class Care_net(nn.Module):
    def __init__(self, input_nc=2, output_nc=2, ngf=32, norm_layer=nn.BatchNorm2d):
        ...
        conv2 = [
            ...
            downrelu,
            nn.Conv2d(
                ngf * 2, ngf * 2, kernel_size=3, stride=1, padding=1, bias=use_bias
            ),
            norm_layer(ngf),
            downrelu,
        ]
        ...
        conv3 = [
            nn.Conv2d(
                ngf * 2, ngf * 4, kernel_size=3, stride=1, padding=1, bias=use_bias
            ),
            norm_layer(ngf),
            downrelu,
            nn.Conv2d(
                ngf * 4, ngf * 2, kernel_size=3, stride=1, padding=1, bias=use_bias
            ),
            norm_layer(ngf),
            downrelu,
        ]
        ...
        up2 = [
            nn.Conv2d(
                ngf * 2, ngf * 1, kernel_size=3, stride=1, padding=1, bias=use_bias
            ),
            norm_layer(ngf),
            uprelu,
            nn.Conv2d(ngf, ngf, kernel_size=3, stride=1, padding=1, bias=use_bias),
        ...
        self.final_conv = nn.Conv2d(
            ngf * 1, output_nc, kernel_size=3, stride=1, padding=1, bias=use_bias
        )
...
def intermediate_output(
    ...
    slice_path = os.path.join(train_slices, "xz", f"{slice_idx}.tif")
...
class ImageDataset_numpy(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.paired:
            item_xz = transform_input_img(
                self.xz[(index) % (self.xz.shape[0])], mode1, self.min_v, self.max_v
            )
            item_xy = transform_target_img(
                self.xy[(index) % (self.xy.shape[0])], mode1, self.min_v, self.max_v
            )
            item_xy_lr = transform_target_img(
                self.xy_lr[(index) % (self.xy_lr.shape[0])],
                mode1,
                self.min_v,
                self.max_v,
            )
            ...
            item_xz = transform_input_img(
                self.xz[random.randint(0, (self.xz).shape[0] - 1)],
                mode1,
                self.min_v,
                self.max_v,
            )
            item_xy = transform_target_img(
                self.xy[(index) % (self.xy.shape[0])], mode2, self.min_v, self.max_v
            )
            item_xy_lr = transform_target_img(
                self.xy_lr[(index) % (self.xy_lr.shape[0])],
                mode2,
...
def create_train_data(train_data_path, batch_size, min_v, max_v):
    ...
    train_data = torch.utils.data.DataLoader(
        ImageDataset_numpy(dataset, False, min_v, max_v),
...
def train_self_net(
    ...
    SSIM_loss = SSIM(
        data_range=1, size_average=True, win_size=11, win_sigma=1.5, channel=1
    )
    ...
    optim_deblur = optim.Adam(
        deblur_net.parameters(), lr=learning_rate_G, betas=(beta1, beta2)
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--input_dir", type=str, required=True, help="Directory for input data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_v", type=int, default=65535, help="Maximum intensity value"
    )
    parser.add_argument(
        "--bg_threshold",
        type=int,
        default=1000,
        help="Background threshold for patch selection",
    )
    parser.add_argument(
        "--scale", type=float, default=0.5, help="Scale factor for XY to Z"
    )
    ...
    parser.add_argument(
        "--log_interval", type=int, default=1, help="Log interval for training"
    )
    parser.add_argument(
        "--imshow_interval",
        type=int,
        default=100,
        help="Image show interval for training",
    )
